...
Imports DlaWebCtrls = ImageList.Deadline.WebControls
...
Partial Class Options
    ...
    Public Sub CreateOption(ByVal pGroupID As Integer, ByVal pOptionName As String, ByVal pOptionType As Char, _
        ...
        Select Case pOptionType
            ...
            Case "s"
                _OptionType = "Swatch"
                _Control = New DlaWebCtrls.ImageList
                _ControlPrice = New CheckBoxList
                AddHandler CType(_Control, DlaWebCtrls.ImageList).SelectedIndexChanged, AddressOf indxChanged
                CType(_Control, DlaWebCtrls.ImageList).CssClass = "imagelist"
            Case Else
                Exit Sub
        End Select
    ...
    Public Sub NewItem(ByVal pItemText As String, ByVal pItemValue As String, _
        ...
        If _Control.GetType = GetType(DlaWebCtrls.ImageList) Then
            ' The control is an image list.
            Dim FilePaths As String() = Directory.GetFiles(Server.MapPath(CkartrisImages.CreateFolderURL(CkartrisImages.IMAGE_TYPE.enum_OptionSwatch, pItemValue)))
            If FilePaths.Count > 0 Then
                itemListOptions.Attributes.Add("ImageUrl", CkartrisImages.CreateFolderURL(CkartrisImages.IMAGE_TYPE.enum_OptionSwatch, pItemValue) & Path.GetFileName(FilePaths(0)))
            End If
        End If
    ...
    ''' <summary>
    ''' Allow external members to know what the display type is. 
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks>Used to identify if a new color swatch has been selected.</remarks>
    Public ReadOnly Property GetControlType As String
        Get
            If _Control.GetType = GetType(DlaWebCtrls.ImageList) Then
                Return "Swatch"
            ElseIf _Control.GetType = GetType(RadioButtonList) Then
                Return "Radio"
            ElseIf _Control.GetType = GetType(CheckBoxList) Then
                Return "Check"
            ElseIf _Control.GetType = GetType(DropDownList) Then
                Return "DropDown"
            Else
                Return String.Empty
            End If
