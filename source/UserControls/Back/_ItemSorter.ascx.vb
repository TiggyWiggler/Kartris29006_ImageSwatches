...
Partial Class _ItemSorter
    ...
    ''' <summary>
    ''' Memory of path to the folder where the image(s) are stored.
    ''' </summary>
    ''' <remarks>DO NOT use directly. Read Property FolderPath instead</remarks>
    Dim c_FolderPath As String = String.Empty
    ...
    ''' <summary>
    ''' Path to the folder where the image(s) are stored.
    ''' </summary>
    ''' <value></value>
    ''' <returns>string.empty if nothing found</returns>
    ''' <remarks>Cached in Viewstate</remarks>
    Public Property FolderPath() As String
        Get
            If c_FolderPath = String.Empty Then
                If Not IsNothing(ViewState("c_FolderPath")) Then
                    ' If c_FolderPath has not been initialised, Viewstate is available
                    c_FolderPath = ViewState("c_FolderPath")
                End If
            End If
        ...
        Set(ByVal value As String)
            c_FolderPath = value
            ViewState("c_FolderPath") = c_FolderPath
        End Set
    End Property
    ...
    Public Sub LoadItemsInSorter()
        If Not Page.IsPostBack Or lbxImagesOrder.Items.Count = 0 Then
            ...
            Dim dirImages As New DirectoryInfo(Server.MapPath(FolderPath))
            ...
            For i As Integer = 0 To fleImage.GetUpperBound(0)
                strImgPath = FolderPath & fleImage(i).Name '& "?nocache=" & Now.Hour & Now.Minute & Now.Second
    ...
    Private Sub RenameItems()
        ...
        For i As Byte = 0 To lbxImagesOrder.Items.Count - 1
            ...
            File.Copy(Server.MapPath(FolderPath & lbxImagesOrder.Items(i).Text), _
                      Server.MapPath(FolderPath & strName))
        Next

        Dim dirInfo As New DirectoryInfo(Server.MapPath(FolderPath))
        Dim fleImage() As FileInfo
        fleImage = dirInfo.GetFiles()
        lbxImagesOrder.Items.Clear()
        For i As Integer = 0 To fleImage.GetUpperBound(0)
            If Not fleImage(i).Name.StartsWith("temp-") Then
                File.SetAttributes(Server.MapPath(FolderPath & fleImage(i).Name), FileAttributes.Normal)
                File.Delete(Server.MapPath(FolderPath & fleImage(i).Name))
            End If
        Next

        For i As Integer = 0 To fleImage.GetUpperBound(0)
            If fleImage(i).Name.StartsWith("temp-") Then
                File.Move(Server.MapPath(FolderPath & fleImage(i).Name), Server.MapPath(FolderPath & fleImage(i).Name.Replace("temp-", "")))
    ...
    Private Sub ReorderImages()
        ...
        If Not Directory.Exists(Server.MapPath(FolderPath)) Then Return
    ...
    Protected Sub rptImages_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptImages.ItemCommand
        ...
        If e.CommandName = "Remove" Then
            ...
            ' Craig Moore 20161019. These two lines and the extra text in the markup are causing me problems elsewhere and they appear to be redundant, 
            ' so I have removed them from here and simplified the code in the line below.
            'imgToRemove.ImageUrl = CType(e.Item.FindControl("litImgURL"), Literal).Text
            'litImgName3.Text = imgToRemove.ImageUrl.ToString
            litImgName3.Text = CType(e.Item.FindControl("litImgURL"), Literal).Text
            popExtender.Show()
        End If
    ...
    Protected Sub lnkYes_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkYes.Click
        Try
            ...
            ' Simplification of the code by removing uneccessary control.
            strFileName = litImgName3.Text ' imgToRemove.ImageUrl
