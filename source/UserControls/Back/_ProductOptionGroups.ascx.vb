...
Partial Class _ProductOptionGroups
    ...
    ''' <summary>
    ''' Creates a datatable that has every possible combination of swatch type images for the current product.
    ''' </summary>
    ''' <returns>Datatable ordered by OptionGroupId, OptionId Ascending</returns>
    ''' <remarks>Author Craig Moore - this would have been much cleaner in the tables, but this way I avoid a change to the data access layer</remarks>
    Private Function GenerateOptionImageCombinations() As DataTable
        Dim Lang As Integer = 1
        If Not IsNothing(Session("LANG")) Then
            Lang = Session("LANG")
        End If

        Dim dtOptionGroups As DataTable= VersionsBLL.GetProductOptions(CInt(litProductID.Text), Lang)

        dtOptionGroups.DefaultView.Sort = "OPTG_ID DESC"     ' Orders the table by ID descending, this results in teh smaller numbered folders closer to the root.

        ' Create a table to store the output structure
        Dim dtOptionGroupValues As DataTable

        Dim dtOptionImages As New DataTable     ' Final table
        dtOptionImages.Columns.Add("OptionImage_ID", GetType(String))
        dtOptionImages.Columns.Add("OptionImage_Name", GetType(String))
        dtOptionImages.Columns.Add("OptionImage_Count", GetType(Integer))
        dtOptionImages.Columns.Add("Option_ID", GetType(Integer))
        dtOptionImages.Columns.Add("OptionGroup_ID", GetType(Integer))

        ' Create a temporary table identical to the output table
        ' used for itterations of the loop (below)
        Dim dtTemp As DataTable = dtOptionImages.Clone

        ' Itteration working objects
        Dim drOptionGroup As DataRow
        Dim drValue As DataRow
        Dim FileCount As Integer

        For Each dvOptionGroup As DataRowView In dtOptionGroups.DefaultView
            drOptionGroup = dvOptionGroup.Row
            If drOptionGroup("OPTG_OptionDisplayType") = "s" Then
                ' Swatch option
                ' Get the option values for this option group
                dtOptionGroupValues = VersionsBLL.GetProductOptionValues(CInt(litProductID.Text), Lang, CInt(drOptionGroup("OPTG_ID")))
                dtOptionGroupValues.DefaultView.Sort = "OPT_ID ASC"     ' Ascending order for the actual options
                dtOptionImages.Clear()   ' Clear any data in the output table. The temp table will now hold anything already generated.
                For Each dvValue As DataRowView In dtOptionGroupValues.DefaultView
                    drValue = dvValue.Row
                    ' itterate through the options in this option group
                    FileCount = GetFileCount(CkartrisImages.IMAGE_TYPE.enum_ProductImage, CInt(litProductID.Text), CStr(drValue("OPT_ID")))
                    dtOptionImages.Rows.Add(CStr(drValue("OPT_ID")),
                                            CStr(drValue("OPT_Name")),
                                            FileCount,
                                            CInt(drValue("OPT_ID")),
                                            CInt(drOptionGroup("OPTG_ID")))
                    For Each drTemp As DataRow In dtTemp.Rows
                        ' Append the temp table values onto the end of the string
                        FileCount = GetFileCount(CkartrisImages.IMAGE_TYPE.enum_ProductImage, CInt(litProductID.Text), CStr(drValue("OPT_ID")) & "/" & drTemp("OptionImage_ID"))
                        dtOptionImages.Rows.Add(CStr(drValue("OPT_ID")) & "/" & drTemp("OptionImage_ID"),
                                                CStr(drValue("OPT_Name")) & ", " & drTemp("OptionImage_Name"),
                                                FileCount,
                                                CInt(drValue("OPT_ID")),
                                                CInt(drOptionGroup("OPTG_ID")))
                    Next
                ...
                dtTemp.Clear()
                For Each r As DataRow In dtOptionImages.Rows
                    dtTemp.Rows.Add(r.ItemArray)
                Next r
            End If
        Next dvOptionGroup
        
        Return dtOptionImages

    End Function

    ''' <summary>
    ''' Read number of files in a folder.
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetFileCount(ByVal pImageType As CkartrisImages.IMAGE_TYPE, ByVal pItemID As String, ByVal SubFolder As String) As Integer
        GetFileCount = 0
        Dim strFolder As String = CkartrisImages.CreateFolderURL(pImageType, pItemID)
        If strFolder.Length > 0 Then
            If Right(strFolder, 1) <> "/" Then
                ' Add trailing slash
                strFolder = strFolder & "/"
            End If
            If SubFolder.Length > 1 Then
                ' Remove leading slash on subfolder
                If Left(SubFolder, 1) = "/" Then
                    SubFolder = Right(SubFolder, SubFolder.Length - 1)
                End If
            End If
            ' Folders below strfolder
            strFolder = strFolder & SubFolder
            Dim dirInfo As DirectoryInfo
            dirInfo = New DirectoryInfo(HttpContext.Current.Server.MapPath(strFolder))
    ...
    ''' <summary>
    '''  Display all of the possible option image combinations along with their counts
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub GetOptionImages()
        dlOptionImages.DataSource = GenerateOptionImageCombinations()
        dlOptionImages.DataBind()
    End Sub
    ...
    Protected Sub rptProductOptions_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptProductOptions.ItemCommand
        ...
        Select Case e.CommandName
            ...
            Case "save"
                ...
                GetOptionImages()           ' Update the option image display
                updCombinations.Update()
            Case "expand"
                ItemExpanded(e.Item)
        End Select
    ...
    Protected Sub dlOptionImages_ItemCommand(source As Object, e As DataListCommandEventArgs) Handles dlOptionImages.ItemCommand
        If e.CommandName = "edit" Then
            ' Show image control so that user can upload Option Images
            dlOptionImages.SelectedIndex = e.Item.ItemIndex
            GetOptionImages()
        End If
    ...
    Protected Sub dlOptionImages_ItemDataBound(sender As Object, e As DataListItemEventArgs) Handles dlOptionImages.ItemDataBound
        If e.Item.ItemType = ListItemType.SelectedItem Then
            ' This is the selected row.
            Dim Uploader As _FileUploader = CType(e.Item.FindControl("OptionImageUploader"), _FileUploader)
            If Not IsNothing(Uploader) Then
                ' Show images in the image uploader. Also populates the folder variable in the control.
                Uploader.LoadImages()
            End If
        End If
    End Sub
