...
<asp:UpdatePanel ID="updMain" runat="server" UpdateMode="Conditional">
    <ContentTemplate>
        ...
        <ajaxToolkit:TabContainer ID="tabContainerMain" runat="server" CssClass=".tab" EnableTheming="False">
            ...
            <ajaxToolkit:TabPanel ID="tabOptionImages" runat="server">
                <HeaderTemplate><span>Option Images</span></HeaderTemplate>
                <ContentTemplate>
                    <div id="page_optiongroups">
                    <asp:DataList ID="dlOptionImages" CssClass="kartristable" runat="server">
                ...
                <HeaderStyle CssClass="header" />
                        <HeaderTemplate>
                            <div class="column_displaytype" style="width: 55px;">
                                <asp:Literal ID="litID" runat="server" Text='ID'></asp:Literal>
                            </div>
                            <div class="column_optionname">
                                <asp:Literal ID="litContentTextName" runat="server" Text='Option Image Name'></asp:Literal>
                            </div>
                            <div class="column_displaytype" style="width: 130px;">
                                <asp:Literal ID="litFormLabelOrderByValue1" runat="server" Text='Image Count' />
                            </div>
                        </HeaderTemplate>
                        <ItemTemplate>
                            <div class="column_displaytype" style="width: 55px;">
                                <asp:Literal ID="litOptionGrpID" runat="server" Text='<%# Eval("OptionImage_ID") %>' Visible="true"></asp:Literal>
                            </div>
                            <div class="column_optionname">
                                <!-- Linkbutton displaying the name e.g. Blue, Striped -->
                                <asp:LinkButton ID="lnkBtnGrpName" runat="server" Text='<%# Eval("OptionImage_Name") %>'
                                    CommandArgument='<%# Eval("OptionImage_ID") %>' CommandName="edit" />
                            </div>
                            <div class="column_displaytype" style="width: 130px;">
                                <!-- How many images are already associated with this grouping -->
                                <asp:Literal ID="litOrderByValue1" runat="server" Text='<%# Eval("OptionImage_Count") %>' />
                            </div>
                            <div class="floatright">
                                <asp:LinkButton ID="lnkBtnGrpNameEdit" CssClass="linkbutton icon_edit" runat="server"
                                    Text='<%$ Resources: _Kartris, FormButton_Edit %>' CommandArgument='<%# Eval("OptionImage_ID") %>'
                                    CommandName="edit" />
                            </div>
                        </ItemTemplate>
                        <SelectedItemTemplate>
                            <h2>Images for product <asp:Literal ID="Literal1" runat="server" Text='<%# Eval("OptionImage_Name") %>'></asp:Literal></h2>
                            <_user:_FileUploader runat="server" id="OptionImageUploader" ImageType="enum_ProductImage" ItemID='<%# litProductID.Text%>' ParentID='<%# Eval("OptionGroup_ID") %>'  imagepath='<%# Eval("OptionImage_ID") %>'/>
                        </SelectedItemTemplate>
                    </asp:DataList>
                        </div>
                </ContentTemplate>
