...
<script runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Catch
        End Try
        ' Modified from number to string by Craig Moore 31-10/2016 so that option groups within the images folder could be 
        ' defined and loaded.
        Dim strItem As String = ""
        Try
            strItem = Request.QueryString("numItem") 'ID number of product/category/version. 
        Catch
        End Try
        ...
        If strFullPath = "" Then
            ...
            Select Case strItemType
                Case "p"
                    strFolderPath = "\Products\" & strItem & "\"
                Case "c"
                    strFolderPath = "\Categories\" & strItem & "\"
                Case "v"
                    strFolderPath = "\Products\" & strParent & "\" & strItem & "\"
                Case "s" 's for "specials" as 'p' is already taken!
                    strFolderPath = "\Promotions\" & strItem & "\"
            End Select

            'if whole directory, scan for suitable image
            If strFileName = "" Then
                For i = 0 To UBound(aryFileTypes)
                    strImagePathToTry = Server.MapPath("images") & strFolderPath & strItem & Trim(aryFileTypes(i))
